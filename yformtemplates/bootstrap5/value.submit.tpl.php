...
foreach ($labels as $index => $label) {
    ...
    echo '<button onclick="if(this.form.checkValidty) { this.disabled = true }" class="' . implode(' ', $classes) . '" type="submit" name="' . $this->getFieldName() . '" id="'. $id .'" value="' . htmlspecialchars($label) . '">' . $label_translated . '</button>';
}
